...
- Purchase-order receiving accepts supplier over-deliveries beyond the original outstanding quantity. The original ordered quantity remains unchanged, excess units are recorded on the PO, receipt history, print summary, and audit trail, and all accepted units are added to inventory at the actual receipt cost.
- App branding uses the official, unchanged KDPLUS Pharmacy logo on authentication, back-office, and POS screens. Browser tabs and installed-window titles use “KDPlus POS” instead of the Emergent placeholder title.
- Product CSV import/export uses the complete product schema, including pharmacy details, units, classifications, pricing, tax, reorder controls, stock-tracking flags, status, and initial lot/expiry data. Older compact import files remain supported through header aliases.
